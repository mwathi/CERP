	...
	public function save() {
		...
		if ($valid == false) {
			...
			if ($num > 0) {
				...
				$transaction -> Ending_Balance = ($account_balance - $gross_earnings);
				$transaction -> save();

				$this -> load -> database();
				$sqlupdatepartakings = "UPDATE partakings SET bank_account = '" . $bank_related_account_credited . "', 
										transaction_value = '" . ($account_balance - $gross_earnings) . "', date =" . date('Y-m-d') . " WHERE bank_account =" . $bank_related_account_credited;
